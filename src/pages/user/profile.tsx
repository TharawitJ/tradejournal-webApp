...
import { updateUserProfile } from "../../api/apiMain";
...
const ProfilePage: React.FC = () => {
  ...
  // Sample models - in a real app these would come from a store or API
  const [models, setModels] = useState(["Breakout", "Mean Reversion", "Trend Following"]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div className="md:col-span-2 space-y-12">
            <section className="bg-[#131313] rounded-3xl p-8 border border-gray-800/50 shadow-xl">
              <div className="flex justify-between items-center mb-8">
                <h2 className="font-headline font-bold text-xl uppercase tracking-tight">Account Details</h2>
                <button 
                  onClick={() => {
                    if (!isEditing) {
                      setEditData({
              ...
              <div className="space-y-6">
                <div>
                  <label className="block font-label text-[11px] uppercase tracking-[0.2em] text-[#adaaaa] mb-2">Username</label>
                  {isEditing ? (
                    <input 
                      type="text" 
                    ...
                    <p className="text-xl font-medium">{user?.username || "Username"}</p>
                  )}
                </div>

                <div>
                  <label className="block font-label text-[11px] uppercase tracking-[0.2em] text-[#adaaaa] mb-2">Email Address</label>
                  {isEditing ? (
                    <input 
                      type="email" 
                    ...
                    <p className="text-xl font-medium">{user?.email || "trader@example.com"}</p>
                  )}
                </div>
            ...
            <section className="bg-[#131313] rounded-3xl p-8 border border-gray-800/50 shadow-xl">
              <div className="flex justify-between items-center mb-8">
                <h2 className="font-headline font-bold text-xl uppercase tracking-tight">Model Setup</h2>
                <button 
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {models.map((model, index) => (
                  <div key={index} className="bg-[#1a1919] border border-gray-800 p-4 rounded-2xl flex justify-between items-center group hover:border-[#9cff93]/30 transition-all">
                    <span className="font-medium">{model}</span>
                    <button 
                      onClick={() => setModels(models.filter((_, i) => i !== index))}
                      className="text-gray-600 hover:text-[#ff716c] transition-colors"
                    >
                      <span className="material-symbols-outlined text-lg text-[#ff716c]">delete</span>
                    </button>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-8">
            <section className="bg-[#131313] border border-red-900/20 rounded-3xl p-8 shadow-xl">
              <h2 className="font-headline font-bold text-lg uppercase tracking-tight text-[#ff716c] mb-6">Danger Zone</h2>
          ...
          <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[100] flex items-center justify-center p-6">
            <div className="bg-[#131313] border-2 border-red-900/30 rounded-3xl p-10 max-w-md w-full shadow-[0_0_64px_rgba(255,113,108,0.1)]">
              <div className="flex justify-center mb-6">
                <span className="material-symbols-outlined text-6xl text-[#ff716c]">warning</span>
              </div>
              <h2 className="text-2xl font-headline font-bold mb-4 text-center uppercase tracking-tight">Confirm Deletion</h2>
              <p className="text-[#adaaaa] text-center mb-10 leading-relaxed">
                Are you absolutely sure? All your journals, charts, and performance history will be wiped from our servers.
              </p>
              <div className="flex flex-col gap-4">
                ...
                <button 
                  className="py-4 rounded-xl font-label font-bold uppercase tracking-widest text-sm bg-[#ff716c] text-white hover:brightness-110 transition-all shadow-lg shadow-red-900/20"
                >
